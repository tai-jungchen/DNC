...
samples will have better classification performance. The feature used here is uni-variate.
"""
import sys
import numpy as np
import pandas as pd
...
def sep(model_type, sub, n_reps):
    ...
    log_file = 'log/' + model_type + '_' + str(sub) + '_' + '.txt'
    csv_file = 'results/' + model_type + '_' + str(sub) + '_' + '.csv'
    sys.stdout = open(log_file, "w")

    # initialize metric dictionary
    record_metrics = ['kl_bin', 'tstat_bin', 'prec_bin', 'rec_bin', 'spec_bin', 'bacc_bin', 'acc_bin', 'f1_bin', 'kappa_bin',
                      'kl_sub', 'tstat_sub', 'prec_sub', 'rec_sub', 'spec_sub', 'bacc_sub', 'acc_sub', 'f1_sub', 'kappa_sub']
    ...
    for i in range(n_reps):
        ...
        # calculate distance
        # kl divergence
        metrics['kl_bin'].append(round(kl_calculator(X_train[(X_train['failure.type'] != 0)]['torque'],
                                                       X_train[(X_train['failure.type'] == 0)]['torque']), 4))
        metrics['kl_sub'].append(round(kl_calculator(X_train[(X_train['failure.type'] == sub)]['torque'],
                                                       X_train[(X_train['failure.type'] == 0)]['torque']), 4))

        # t statistics
        t_stat_bin, p_value = ttest_ind(X_train[(X_train['failure.type'] != 0)]['torque'],
                                        X_train[(X_train['failure.type'] == 0)]['torque'])
        metrics['tstat_bin'].append(round(t_stat_bin, 4))
        t_stat_sub, p_value = ttest_ind(X_train[(X_train['failure.type'] == sub)]['torque'],
                                        X_train[(X_train['failure.type'] == 0)]['torque'])
        metrics['tstat_sub'].append(round(t_stat_sub, 4))
        ...
        # training
        bin_model.fit(X_train[['torque']], y_train)
        model.fit(X_train_fk[['torque']], y_train_fk)

        # testing
        # y_train_pred_bin = bin_model.predict(X_train.drop(columns='failure.type'))
        y_test_pred_bin = bin_model.predict(X_test_fk[['torque']])
        # y_train_pred_sub = model.predict(X_train_fk[['torque']])
        y_test_pred_sub = model.predict(X_test_fk[['torque']])
